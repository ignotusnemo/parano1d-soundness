...
pub fn history_local_certificate(
    ...
    let candidate_switching_exception = ExactProbability::dyadic(
        &maximum_initial_list_size_bound * parameters.history_max_algebraic_roots,
        parameters.challenge_min_entropy_bits,
    );
    let joint_sidecar_exception = ExactProbability::dyadic(
        &maximum_initial_list_size_bound * parameters.history_joint_sidecar_roots,
        parameters.challenge_min_entropy_bits,
    );
    let local_rbr = [
        query_escape.clone(),
...
mod tests {
    ...
    #[test]
    fn algebraic_root_bounds_follow_the_snapshot() {
        let parameters = ProductionParameters::load().unwrap();
